...
import type { Purchase, FindPurchasesQuery, PurchaseStatus } from '../../types/purchases';
...
import DeletePurchaseConfirmModal from '../../components/purchases/DeletePurchaseConfirmModal';
...
const PurchasesPage: React.FC = () => {
  ...
  const [query, setQuery] = useState<FindPurchasesQuery>({
    page: 1,
    limit: 10,
    search: '',
    status: undefined,
  ...
  const fetchPurchases = useCallback(async () => {
    ...
    try {
      setLoading(true);
      setError(null);
      const data = await getPurchases(query);
  ...
  const handleStatusChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    setQuery(prev => ({ ...prev, status: val ? (val as PurchaseStatus) : undefined, page: 1 }));
  };
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 md:px-8">
      <div className="flex justify-between items-center mb-6">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-500">Manage purchase orders and supplier purchases.</p>
        </div>
        {canCreate && (
          <button
                ...
                {purchases.map((item) => (
                  <tr key={item.id}>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="text-sm font-medium text-blue-600">{item.purchaseNumber}</div>
                      <div className="text-xs text-gray-500">{item.orderDate ? new Date(item.orderDate).toLocaleDateString() : '-'}</div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${
                        ...
                        item.status === 'APPROVED' || item.status === 'ORDERED' ? 'bg-blue-100 text-blue-800' :
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium text-gray-900">
                      {item.currency} {Number(item.grandTotal).toFixed(2)}
                    </td>
