...
const DeleteCustomerConfirmModal: React.FC<DeleteCustomerConfirmModalProps> = ({
  isOpen,
  onClose,
  onSuccess,
  customer,
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      setError(err.response?.data?.message || err.message || 'Failed to deactivate customer.');
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
        ...
        <div className="inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg w-full relative z-10">
              ...
              <div className="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                ...
                <div className="mt-2">
                  <p className="text-sm text-gray-500">
                    Are you sure you want to deactivate <span className="font-semibold text-gray-800">{customer.firstName} {customer.lastName} ({customer.customerCode})</span>?
                    This will mark the customer as INACTIVE. Note that if the customer has existing sales, they cannot be permanently deleted.
                  </p>
                  {error && (
          ...
          <div className="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
            ...
            >
              {loading ? 'Deactivating...' : 'Deactivate Customer'}
            </button>
            <button
              type="button"
              disabled={loading}
