...
import type { GoodsReceipt } from '../../types/goodsReceipts';
import { getGoodsReceiptById } from '../../api/goodsReceipts.api';
...
const GoodsReceiptDetailsModal: React.FC<GoodsReceiptDetailsModalProps> = ({
  isOpen,
  onClose,
  receiptId,
}) => {
  ...
  const fetchReceipt = async () => {
    try {
      ...
      setError(err.response?.data?.message || err.message || 'Failed to load receipt details');
    } finally {
      setLoading(false);
    }
  };
              ...
              <h3 className="text-lg leading-6 font-medium text-gray-900 flex items-center">
                ...
                {receipt && (
                  <span className={`ml-3 px-3 py-1 inline-flex text-xs leading-5 font-semibold rounded-full ${
                    ...
                    receipt.status === 'CANCELLED' ? 'bg-red-100 text-red-800' : 'bg-gray-100 text-gray-800'
                  }`}>
                    {receipt.status}
                  </span>
                )}
            ...
            {loading ? (
              ...
              <div className="space-y-6">
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4 bg-gray-50 p-4 rounded-lg">
                  <div>
                    <span className="block text-xs font-medium text-gray-500 uppercase">Receipt Number</span>
                    <span className="block mt-1 text-sm text-gray-900 font-semibold">{receipt.receiptNumber}</span>
                  </div>
                  <div>
                    <span className="block text-xs font-medium text-gray-500 uppercase">Purchase Number</span>
                    <span className="block mt-1 text-sm text-gray-900 font-semibold">{receipt.purchase?.purchaseNumber || receipt.purchaseId}</span>
                  </div>
                  <div>
                    <span className="block text-xs font-medium text-gray-500 uppercase">Received Date</span>
                    <span className="block mt-1 text-sm text-gray-900">{new Date(receipt.receivedAt).toLocaleString()}</span>
                  </div>
                  <div>
                    <span className="block text-xs font-medium text-gray-500 uppercase">Received By</span>
                    <span className="block mt-1 text-sm text-gray-900">{receipt.receivedByUser ? `${receipt.receivedByUser.firstName} ${receipt.receivedByUser.lastName}` : receipt.receivedBy}</span>
                  </div>
                  <div>
                    <span className="block text-xs font-medium text-gray-500 uppercase">Store</span>
                    <span className="block mt-1 text-sm text-gray-900">{receipt.store?.name || receipt.storeId}</span>
                  </div>
                </div>

                <div>
                  <h4 className="text-md font-medium text-gray-900 mb-3 border-b pb-2">Received Items</h4>
                  <div className="overflow-x-auto border border-gray-200 rounded-md">
                    <table className="min-w-full divide-y divide-gray-200">
                      <thead className="bg-gray-50">
                        <tr>
                          <th className="px-4 py-2 text-left text-xs font-medium text-gray-500 uppercase">Product</th>
                          <th className="px-4 py-2 text-right text-xs font-medium text-gray-500 uppercase">Received Qty</th>
                      ...
                      <tbody className="bg-white divide-y divide-gray-200">
                        {receipt.items?.map((item) => (
                          <tr key={item.id}>
                            ...
                            <td className="px-4 py-2 whitespace-nowrap text-right text-sm font-bold text-gray-900">{Number(item.receivedQuantity).toFixed(2)}</td>
